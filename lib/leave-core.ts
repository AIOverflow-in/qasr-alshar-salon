/**
 * Pure leave-balance math — 21 days/year, accruing only after 12 months of service.
 *
 * Extracted from lib/leave.ts (which is `server-only`) so it's unit-testable. lib/leave.ts
 * re-exports everything here, so existing `@/lib/leave` imports are unchanged.
 */
