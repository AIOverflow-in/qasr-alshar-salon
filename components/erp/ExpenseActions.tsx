  ...
  return (
    <>
      <div className="flex shrink-0 items-center gap-1">
        ...
        <button onClick={remove} disabled={pending} aria-label="Delete expense" className="-m-1.5 p-1.5 text-muted hover:text-red-400">{pending ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}</button>
      </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-start justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm" onClick={() => setEditing(false)}>
          <div className="mt-10 w-full max-w-md rounded-2xl border border-ink-line bg-ink p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              ...
              <input type="datetime-local" value={f.incurredOn} onChange={(e) => setF((p) => ({ ...p, incurredOn: e.target.value }))} className={`${input} [color-scheme:dark]`} aria-label="Date & time" />
