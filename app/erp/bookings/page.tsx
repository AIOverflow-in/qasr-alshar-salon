  ...
  const [bookings, services, staff, clients, total, statusGroup, sourceGroup, cToday, cTomorrow, cNext2w] =
    await Promise.all([
      prisma.booking.findMany({
        where,
        // For a day view (today/tomorrow) show the schedule in appointment-time order so reception
        // can work it top-to-bottom; for the broader lists keep newest-entry-first.
        orderBy: (when === "today" || when === "tomorrow") ? { startAt: "asc" } : { createdAt: "desc" },
        skip: win.skip,
        take: win.take,
        include: {
